...
class SignupForm(UserCreationForm):
    ...
    resume = forms.ImageField(label="رزومه", required=False)
...
class ProfileForm(forms.ModelForm):
    ...
    job = forms.CharField(max_length=50, label="سمت شغلی", required=False)
    gender = forms.ChoiceField(label="جنسیت", choices=PROFILE_GENDER_CHOICES, required=False)
    nc = forms.CharField(label="کدملی", max_length=11, required=False, widget = forms.TextInput(attrs={'readonly':'readonly'})) #nationalcode
    date_employment = forms.DateTimeField(label="تاریخ عضویت",widget = forms.TextInput(attrs={'readonly':'readonly'}))
    #image = forms.ImageField(label="عکس",upload_to=persons_image_path, validators=[limit_file_size], required=False)
    is_author = forms.ChoiceField(label="آیا نویسنده هستید", choices=PROFILE_AUTHOR_CHOICES, required=True)
    resume = forms.ImageField(label="رزومه", required=False)
